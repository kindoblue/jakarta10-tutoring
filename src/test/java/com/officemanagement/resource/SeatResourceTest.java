...
public class SeatResourceTest extends BaseResourceTest {
    ...
    public void testCreateAndGetSeat() {
        ...
        QuarkusTransaction.run(() -> {
            Floor floor = new Floor(); floor.setName("API Test Floor - Seat"); floor.setFloorNumber(201); testEntityManager.persist(floor);
            OfficeRoom room = new OfficeRoom(); room.setName("API Test Room - Seat"); room.setRoomNumber("SRoom1"); room.setFloor(floor); testEntityManager.persist(room);
        ...
        .extract().as(SeatDTO.class); // Extract DTO
    ...
    public void testCreateSeatNoRoom() {
        ...
        given().contentType(ContentType.JSON).body(seat).when().post("/seats")
    ...
    public void testCreateSeatDuplicateNumberInRoom() {
        ...
        final Holder<Long> roomIdHolder = new Holder<>();
         QuarkusTransaction.run(() -> {
            Floor floor = new Floor(); floor.setName("API Test Floor - DupSeat"); floor.setFloorNumber(203); testEntityManager.persist(floor);
            OfficeRoom room = new OfficeRoom(); room.setName("API Test Room - DupSeat"); room.setRoomNumber("R-DupS"); room.setFloor(floor); testEntityManager.persist(room);
            testEntityManager.flush();
            roomIdHolder.value = room.getId();
        });
        Long roomId = roomIdHolder.value;
        ...
        OfficeRoom roomRef1 = new OfficeRoom(); roomRef1.setId(roomId);
        seat1Payload.setRoom(roomRef1);
        given().contentType(ContentType.JSON).body(seat1Payload).when().post("/seats")
               .then().log().ifValidationFails().statusCode(Response.Status.CREATED.getStatusCode());

        // Create Seat 2 (duplicate number in same room)
        Seat seat2Payload = new Seat();
        seat2Payload.setSeatNumber("DupS1"); // Same number
        OfficeRoom roomRef2 = new OfficeRoom(); roomRef2.setId(roomId);
        seat2Payload.setRoom(roomRef2);
        given().contentType(ContentType.JSON).body(seat2Payload).when().post("/seats")
    ...
    public void testUpdateSeat() {
        ...
        QuarkusTransaction.run(() -> {
            Floor floor = new Floor(); floor.setName("API Floor - UpdSeat"); floor.setFloorNumber(204); testEntityManager.persist(floor);
            OfficeRoom room = new OfficeRoom(); room.setName("API Room - UpdSeat"); room.setRoomNumber("R-UpdS"); room.setFloor(floor); testEntityManager.persist(room);
            Seat seat = new Seat(); seat.setSeatNumber("OrigS1"); seat.setRoom(room); testEntityManager.persist(seat);
            testEntityManager.flush();
            roomIdHolder.value = room.getId();
        ...
        assertNotNull(roomId); assertNotNull(seatId);
        ...
        OfficeRoom roomRef = new OfficeRoom(); roomRef.setId(roomId); // Reference existing room
        updatePayload.setRoom(roomRef); 

        // Assert PUT response (expecting SeatDTO)
        given().contentType(ContentType.JSON).body(updatePayload).when().put("/seats/" + seatId)
    ...
    public void testDeleteSeat() {
        ...
        final Holder<Long> seatIdHolder = new Holder<>();
        QuarkusTransaction.run(() -> {
            Floor floor = new Floor(); floor.setName("API Floor - DelSeat"); floor.setFloorNumber(205); testEntityManager.persist(floor);
            OfficeRoom room = new OfficeRoom(); room.setName("API Room - DelSeat"); room.setRoomNumber("R-DelS"); room.setFloor(floor); testEntityManager.persist(room);
            Seat seat = new Seat(); seat.setSeatNumber("DeleteS1"); seat.setRoom(room); testEntityManager.persist(seat);
            testEntityManager.flush();
            seatIdHolder.value = seat.getId();
        });
        Long seatId = seatIdHolder.value;
    ...
    public void testCreateSeatWithInvalidData() {
        ...
        OfficeRoom roomRef = new OfficeRoom(); roomRef.setId(1L); // Assume room 1 exists or use valid ID
        ...
        seat2.setSeatNumber("ValidNum"); // Missing room
         given().contentType(ContentType.JSON).body(seat2).when().post("/seats")
            .then().statusCode(Response.Status.BAD_REQUEST.getStatusCode());
    }
    ...
    public void testDeleteSeatWithEmployeesAssigned() {
        ...
        final Holder<Long> seatIdHolder = new Holder<>();
        QuarkusTransaction.run(() -> {
            Floor floor = new Floor(); floor.setName("API Floor - DelAssignSeat"); floor.setFloorNumber(206); testEntityManager.persist(floor);
            OfficeRoom room = new OfficeRoom(); room.setName("API Room - DelAssignSeat"); room.setRoomNumber("R-DelAS"); room.setFloor(floor); testEntityManager.persist(room);
            Seat seat = new Seat(); seat.setSeatNumber("DelS-Assign"); seat.setRoom(room); testEntityManager.persist(seat);
            Employee employee = new Employee(); employee.setFullName("Assigned Emp"); employee.setOccupation("Worker"); testEntityManager.persist(employee);
            testEntityManager.flush(); // Flush to get IDs
            // Fetch the managed employee to associate the seat
            Employee managedEmployee = testEntityManager.find(Employee.class, employee.getId());
