...
public class SeatResource {
    ...
    public Response getSeat(@PathParam("id") Long id) {
        Seat seat = entityManager.createQuery(
            "SELECT s FROM Seat s LEFT JOIN FETCH s.employees WHERE s.id = :id", Seat.class)
            .setParameter("id", id)
    ...
    public Response createSeat(Seat seat) {
        if (seat == null || seat.getSeatNumber() == null || seat.getSeatNumber().trim().isEmpty()) {
            return Response.status(Response.Status.BAD_REQUEST).entity("Seat number is required").build();
        }
        if (seat.getRoom() == null || seat.getRoom().getId() == null) {
            return Response.status(Response.Status.BAD_REQUEST).entity("Room reference is required").build();
        }
        ...
        Long count = entityManager.createQuery(
            "SELECT COUNT(s) FROM Seat s WHERE s.room.id = :roomId AND s.seatNumber = :seatNumber", Long.class)
    ...
    public Response updateSeat(@PathParam("id") Long id, Seat seatData) {
        if (seatData == null || seatData.getSeatNumber() == null || seatData.getSeatNumber().trim().isEmpty()) {
            return Response.status(Response.Status.BAD_REQUEST).entity("Seat number is required").build();
        }
        ...
        if (!existingSeat.getSeatNumber().equals(seatData.getSeatNumber()) || !existingSeat.getRoom().getId().equals(room.getId())) {
             Long count = entityManager.createQuery(
                "SELECT COUNT(s) FROM Seat s WHERE s.room.id = :roomId AND s.seatNumber = :seatNumber AND s.id != :seatId", Long.class)
                .setParameter("roomId", room.getId())
            ...
            if (count > 0) {
                return Response.status(Response.Status.CONFLICT)
                    .entity("A seat with number " + seatData.getSeatNumber() + " already exists in this room")
                    .build();
            }
        }
