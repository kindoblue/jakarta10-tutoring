...
public class EmployeeResource {
    ...
    public Response getEmployeeSeats(@PathParam("id") Long id) {
        ...
        Employee employee = entityManager.createQuery(
            ...
            Employee.class)
            .setParameter("id", id)
            .getResultStream().findFirst().orElse(null); // Use getResultStream().findFirst().orElse(null)
    ...
    public Response assignSeat(@PathParam("id") Long employeeId, @PathParam("seatId") Long seatId) {
        ...
        Employee employee = entityManager.find(Employee.class, employeeId); // Use find instead of get
        if (employee == null) {
        ...
        // ... existing code ...
        Employee refreshedEmployee = entityManager.createQuery(
            "select e from Employee e " +
            "left join fetch e.seats s " +
            "left join fetch s.room r " +
            "left join fetch r.floor f " +
    ...
    public Response searchEmployees(
        ...
        if (page < 0) {
            return Response.status(Response.Status.BAD_REQUEST).entity("Page index must not be less than zero!").build();
        }
        if (size <= 0) {
            return Response.status(Response.Status.BAD_REQUEST).entity("Page size must not be less than one!").build();
        }
        // Add a reasonable upper limit for size to prevent excessive results
        if (size > 100) { // Example limit
            return Response.status(Response.Status.BAD_REQUEST).entity("Page size must not exceed 100!").build();
        }

        String countQuery = "SELECT COUNT(e) FROM Employee e WHERE LOWER(e.fullName) LIKE LOWER(:searchTerm) OR LOWER(e.occupation) LIKE LOWER(:searchTerm)";
        long totalElements = entityManager.createQuery(countQuery, Long.class)
                .setParameter("searchTerm", "%" + searchTerm + "%")
                .getSingleResult();

        String query = "SELECT e FROM Employee e WHERE LOWER(e.fullName) LIKE LOWER(:searchTerm) OR LOWER(e.occupation) LIKE LOWER(:searchTerm) ORDER BY e.fullName";
