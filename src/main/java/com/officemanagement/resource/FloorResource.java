...
public class FloorResource {
    ...
    public Response getAllFloors() {
        List<Floor> floors = entityManager.createQuery("SELECT DISTINCT f FROM Floor f LEFT JOIN FETCH f.rooms", Floor.class).getResultList();
    ...
    public Response getFloor(@PathParam("id") Long id) {
        Floor floor = entityManager.createQuery(
            "SELECT f FROM Floor f LEFT JOIN FETCH f.rooms WHERE f.id = :id", Floor.class)
            .setParameter("id", id)
    ...
    public Response getFloorPlan(@PathParam("id") Long id) {
        ...
        if (planimetry == null || planimetry.getPlanimetry() == null || planimetry.getPlanimetry().isEmpty()) {
            LOG.warnf("Planimetry not found or empty for floor %d", id);
            return Response.status(Response.Status.NOT_FOUND).entity("No floor plan found for this floor.").build();
        }
    ...
    public Response createFloor(Floor floor) {
        if (floor == null || floor.getName() == null || floor.getName().trim().isEmpty() || floor.getFloorNumber() == null) {
            return Response.status(Response.Status.BAD_REQUEST).entity("Name and floor number are required").build();
        }

        Long count = entityManager.createQuery("SELECT COUNT(f) FROM Floor f WHERE f.floorNumber = :number", Long.class)
                                  .setParameter("number", floor.getFloorNumber())
                                  .getSingleResult();
        if (count > 0) {
            return Response.status(Response.Status.CONFLICT).entity("Floor number " + floor.getFloorNumber() + " already exists.").build();
        }
    ...
    public Response updateFloor(@PathParam("id") Long id, Floor floorData) {
        if (floorData == null || floorData.getName() == null || floorData.getName().trim().isEmpty() || floorData.getFloorNumber() == null) {
            return Response.status(Response.Status.BAD_REQUEST).entity("Name and floor number are required").build();
        }
        ...
        if (!existingFloor.getFloorNumber().equals(floorData.getFloorNumber())) {
            Long count = entityManager.createQuery("SELECT COUNT(f) FROM Floor f WHERE f.floorNumber = :number AND f.id != :id", Long.class)
                                      .setParameter("number", floorData.getFloorNumber())
                                      .setParameter("id", id)
                                      .getSingleResult();
            if (count > 0) {
                return Response.status(Response.Status.CONFLICT).entity("Floor number " + floorData.getFloorNumber() + " already exists.").build();
            }
    ...
    public Response deleteFloor(@PathParam("id") Long id) {
        ...
        Long roomCount = entityManager.createQuery("SELECT COUNT(r) FROM OfficeRoom r WHERE r.floor.id = :floorId", Long.class)
                                      .setParameter("floorId", id)
                                      .getSingleResult();
        if (roomCount > 0) {
            return Response.status(Response.Status.BAD_REQUEST).entity("Cannot delete floor with existing rooms.").build();
        }
    ...
    public Response uploadFloorPlan(@PathParam("id") Long id, String svgData) {
        ...
        if (svgData == null || svgData.trim().isEmpty()) {
            return Response.status(Response.Status.BAD_REQUEST).entity("SVG data cannot be empty").build();
        }
