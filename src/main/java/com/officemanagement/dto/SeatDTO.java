    ...
    public SeatDTO(Seat seat) {
        ...
        if (seat.getRoom() != null) {
            ...
            if (seat.getRoom().getFloor() != null) {
                ...
                this.floorName = seat.getRoom().getFloor().getName(); // Assuming Floor has getName()
            }
        }

        if (seat.getEmployees() != null) {
            // Ensure employees collection is initialized if lazy
            // Hibernate.initialize(seat.getEmployees()); // Potentially needed if called outside transaction
            this.employeeIds = seat.getEmployees().stream()
